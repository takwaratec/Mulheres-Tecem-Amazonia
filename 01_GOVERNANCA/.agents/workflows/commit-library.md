...
1. **Revisão de Conformidade**:
   ...
   - Aplique o workflow `/doi-sync` para garantir que os metadados e badges estejam alinhados à versão v1.0 (WTF).

2. **Sincronização Final**:
   - Garanta que as tags de status (NT, Memorando, etc.) estejam consistentes com a catalogação oficial.
...
4. **Preparação do Release Zenodo**:
   - Após o commit (que deve ser realizado manualmente pelo humano após ler o report), gere um arquivo `.zip` da pasta `docs/` e arquivos de governança para o release Zenodo v1.0 (WTF).
